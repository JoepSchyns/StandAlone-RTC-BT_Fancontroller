...
void TimeAlarmsClass::write(AlarmID_t ID, time_t value,int data)
{
  if (isAllocated(ID)) {
    Alarm[ID].data = data;
...
AlarmID_t TimeAlarmsClass::create(time_t value, OnTick_t onTickHandler, uint8_t isOneShot, dtAlarmPeriod_t alarmType,int data)
{
