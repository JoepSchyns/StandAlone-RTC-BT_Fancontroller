void setupAlarms() {
  ...
  if (memAlarm != NULL ) { //if is a valid alarmClass parameter
    ...
    for (int i = 0; i < dtNBR_ALARMS; i++) {
      ...
      if(Alarm.Alarm[i].Mode.isEnabled){
        Alarm.enable(i);
      }
      
      Serial.println("ID: " + (String)i);
      Serial.println("isEnabled: " + onOrOff(Alarm.Alarm[i].Mode.isEnabled));
      Serial.println("alarmType: " + (String)Alarm.Alarm[i].Mode.alarmType);
      //result += " onTickHandler: " + (String)Alarm.Alarm[i].onTickHandler);
      Serial.println("time: " + time_tReadable(Alarm.Alarm[i].value));
      Serial.println("nextTrigger: " + time_tReadable(Alarm.Alarm[i].nextTrigger));
